...
if ( have_comments() || comments_open() ) {
	...
	?>
		<div class="comments_form_wrap">
			<h2 class="section_title comments_form_title"><?php esc_html_e('Añadir comentario', 'organics'); ?></h2>
			<div class="comments_form">
				...
				$comments_args = array(
						...
						'label_submit'=>esc_html__('Enviar comentario', 'organics'),
						// change the title of the reply section
						'title_reply'=>'',
						// remove "Logged in as"
						'logged_in_as' => '',
						// remove text before textarea
						'comment_notes_before' => '<p class="comments_notes">'.esc_html__('Su dirección de correo electrónico no será publicada. Los campos marcados con * son requeridos', 'organics').'</p>',
						...
						'fields' => apply_filters( 'comment_form_default_fields', array(
							'author' => '<div class="columns_wrap"><div class="comments_field comments_author column-1_1">'
									. '<label for="author"' . ( $req ? ' class="required"' : '' ). '>' . esc_html__( 'Nombre', 'organics' ) . '</label>'
									. '<input id="author" name="author" type="text" placeholder="' . esc_attr__( 'Nombre', 'organics' ) . ( $req ? ' *' : '') . '" value="' . esc_attr( isset($commenter['comment_author']) ? $commenter['comment_author'] : '' ) . '" size="30"' . ($aria_req) . ' />'
									. '</div>',
							...
							'url' => '<div class="comments_field comments_site column-1_1">'
									. '<label for="url" class="optional">' . esc_html__( 'Sitio web', 'organics' ) . '</label>'
									. '<input id="url" name="url" type="text" placeholder="' . esc_attr__( 'Sitio web', 'organics' ) . '" value="' . esc_attr(  isset($commenter['comment_author_url']) ? $commenter['comment_author_url'] : '' ) . '" size="30"' . ($aria_req) . ' />'
									. '</div>'
									. '</div>'
						) ),
                    ...
                    'comment_field' => '<div class="columns_wrap"><div class="comments_field comments_message column-1_1">'
                        ...
                        . '<textarea id="comment" name="comment" placeholder="' . esc_attr__( 'Comentario', 'organics' ) . '" aria-required="true"></textarea>'
                        . '</div>'.'</div>'
				);
