	...
	<div class="form_wrap">
		<div class="form_left">
			<form action="<?php echo wp_login_url(); ?>" method="post" name="login_form" class="popup_form login_form">
				<label for="redirect_to" class="[ hidden ]">redirect_to</label>
				<input type="hidden" name="redirect_to" value="<?php echo esc_attr(home_url()); ?>">
				<div class="popup_form_field login_field iconed_field icon-user"><label for="log" class="[ hidden ]">login</label><input type="text" id="log" name="log" value="" placeholder="<?php esc_attr_e('Login or Email', 'organics'); ?>"></div>
				<div class="popup_form_field password_field iconed_field icon-lock"><label for="password" class="[ hidden ]">password</label><input type="password" id="password" name="pwd" value="" placeholder="<?php esc_attr_e('Password', 'organics'); ?>"></div>
